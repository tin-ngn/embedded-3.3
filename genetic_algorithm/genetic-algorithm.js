...
function getIndex(probabilities) {
  ...
  const ind = probabilities.reduce((acc, rec, index, arr) => {
    if (acc === -1 && r < sumBefore(arr, index + 1)) {
      return index;
    }
    return acc;
  }, -1);
...
function mutate(person, mutationChance) {
  const chance = mutationChance;
...
function getNewPopulation(population, probabilities, mutationChance) {
  return population.map(() => {
    return mutate(crossover(population, probabilities), mutationChance);
  });
}

export function geneticAlgorithm(_args) {
  const ITERATIONS = 200;
  const arr = [];
  let mutationChance = 0;
  for (let i = 0; i < ITERATIONS; i++) {
    const args = _args.slice();
    mutationChance = (0.5 * (i + 1)) / ITERATIONS;
    const t0 = Date.now();
    if (!args[0]) {
      return [];
    }
    ...
    while (!answer.length) {
      ...
      population = getNewPopulation(
        population,
        getProbabilities(fitness),
        mutationChance,
      );
    }
    const t1 = Date.now();
    arr.push([answer, iterations, t1 - t0]);
  }
  ...
  return [...arr[index], (0.3 * (index + 1)) / ITERATIONS];
}
